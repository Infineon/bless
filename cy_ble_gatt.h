...
typedef struct
{
    ...
    /** 
     *  Discovery included service information, used for 128-bit UUID 
     *  read response handle and for continue discovery.
     */
...
/** GATT Service configuration structure (server) */
typedef struct
{
    /** Structure with attributes information */
    const cy_stc_ble_gatts_t *attrInfo;

} cy_stc_ble_gatts_config_t;

/** GATT Service Configuration structure (client) */
typedef struct
{
    /** Structure with Discovered Attributes Information */
    cy_stc_ble_gattc_t *attrInfo;
        
    /** The Discovery Service index */
    uint8_t serviceDiscIdx;

} cy_stc_ble_gattc_config_t;

/** Service event handler callback type */
...
extern const cy_stc_ble_gatts_config_t *cy_ble_gattsConfigPtr;
extern const cy_stc_ble_gattc_config_t *cy_ble_gattcConfigPtr;
